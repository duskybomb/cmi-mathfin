...
full_data <- download_data(file_name, start_date = "2018-01-02", end_date = "2019-05-31")
date <- zoo(c(1:length(full_data[[1]])),as.Date(index(full_data[[1]])))

# Estimation window - Jan 2, 2018 to Apr 10,2019
estim_data <- list()
...
# Event window - Apr 11 - May 31
event_data <- list()
for (i in seq(1,length(sector),1))
{
  event_data[[i]] <- full_data[[i]][c(date[as.Date("2019-05-20")]:date[as.Date("2019-05-31")])]
}  
